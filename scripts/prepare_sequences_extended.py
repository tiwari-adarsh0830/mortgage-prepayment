...
SAVE_DIR   = '/scratch/at7095/mortgage_prepayment/data/sequences_extended'
OOS_DIR    = '/scratch/at7095/mortgage_prepayment/data/sequences_oos'

# Training vintages: 2013Q1-2019Q4 (pre-boom, full 33-month sequences)
VINTAGES = [
...
# OOS holdout: 2020-2021 refi boom — never used in training
OOS_VINTAGES = [
...
def main():
    ...
    # ── Pass 5: build OOS sequences (2020-2021 refi boom, never seen in training) ──
    print('\nPass 5: building OOS sequences (2020-2021 holdout)...', flush=True)
    ...
    for v in OOS_VINTAGES:
        # No keep_ids filter — take all loans from OOS vintages
        df = load_vintage(v, pmms_rates, zhvi_df, keep_ids=None, sample_frac=1.0)
        if df.empty:
            continue
        seq, mask, lbl, pt, lids = build_sequences(df, scaler)
        oos_seq_list.append(seq)
        oos_mask_list.append(mask)
        oos_labels_list.append(lbl)
        oos_prepay_list.append(pt)
        oos_ids_list.append(lids)
        del df, seq, mask, lbl, pt, lids
        gc.collect()

    oos_seq      = np.concatenate(oos_seq_list,    axis=0)
    oos_mask     = np.concatenate(oos_mask_list,   axis=0)
    oos_labels   = np.concatenate(oos_labels_list, axis=0)
    oos_prepay_t = np.concatenate(oos_prepay_list, axis=0)
    oos_loan_ids = np.concatenate(oos_ids_list,    axis=0)
    del oos_seq_list, oos_mask_list, oos_labels_list, oos_prepay_list, oos_ids_list
    gc.collect()

    np.save(os.path.join(OOS_DIR, 'oos_seq.npy'),             oos_seq)
    np.save(os.path.join(OOS_DIR, 'oos_mask.npy'),            oos_mask)
    np.save(os.path.join(OOS_DIR, 'oos_labels.npy'),          oos_labels)
    np.save(os.path.join(OOS_DIR, 'oos_loan_ids.npy'),        oos_loan_ids)
    np.save(os.path.join(OOS_DIR, 'oos_prepay_timestep.npy'), oos_prepay_t)
    print(f'  OOS shape: {oos_seq.shape} — saved to {OOS_DIR}', flush=True)

    prepaid_mask = oos_labels == 1
    found = (oos_prepay_t[prepaid_mask] >= 0).sum()
    print(f'  OOS prepay rate: {prepaid_mask.mean()*100:.2f}% | timestep coverage: {found:,}/{prepaid_mask.sum():,}', flush=True)

    del oos_seq, oos_mask, oos_labels, oos_prepay_t, oos_loan_ids
    gc.collect()
