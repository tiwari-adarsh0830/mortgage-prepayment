...
def build_sequences(df: pd.DataFrame, scaler: StandardScaler):
    ...
    Takes the LAST MAX_SEQ_LEN months per loan chronologically (trailing
    window ending at the cutoff, or at the payoff row for prepaid loans).
    ...
    # TRAILING window: keep the LAST MAX_SEQ_LEN rows per loan, then renumber
    # from 0. Rows are already filtered to <= cutoff, and Fannie stops reporting
    # a loan after its zero-balance row (verified: 121/121 payoff loans in a
    # 2015Q1 slice have zero rows after payoff), so for a prepaid loan the last
    # row IS the payoff row and no event-truncation is needed.
    df['_rev']      = df.groupby('loan_id').cumcount(ascending=False)
    df              = df[df['_rev'] < MAX_SEQ_LEN].copy()
...
def main():
    ...
    SAVE_DIR  = os.path.join(BASE, f'data/sequences_rolling/cutoff_{args.cutoff_year}_zbc_trail' + ('' if args.max_seq_len == _DEFAULT_SEQ_LEN else f'_L{args.max_seq_len}'))
